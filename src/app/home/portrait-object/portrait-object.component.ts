...
export class PortraitObjectComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    //
    // const items = ["Locating the required gigapixels to render...",
    //         "Spinning up the hamster...",
    //         "Shovelling coal into the server...",
    //         "Programming the flux capacitor"];
    // this.sentence.innerText = items[Math.floor(Math.random() * items.length)];
    ...
    objectLoader.load('assets/img/obj/hand.OBJ', ( object ) => {
      // called when the resource is loaded
        if (object) {
          object.rotation.set(0, -3, 1.6);
          scene.add( object );
        }
    },
      ( xhr ) => {
